...
public sealed record GetBalanceByIdQuery(Guid WorkspaceId, Guid BalanceId) : IRequest<Result<BalanceModel>>, IWorkspaceBoundedRequest;
    ...
    IBalanceReadRepository balanceReadRepository) : IRequestHandler<GetBalanceByIdQuery, Result<BalanceModel>>
{
    ...
    {
        var model = await balanceReadRepository.GetById(request.WorkspaceId, request.BalanceId, cancellationToken);
