...
internal sealed class UpdateCategoryEndpoint(ISender sender)
    : Endpoint<
        UpdateCategoryEndpoint.RequestModel,
        Results<Ok, BadRequest<Error>>>
{
    public override void Configure()
    {
        Put("api/workspaces/{workspaceId:guid}/categories/{categoryId:guid}");
        Options(x =>
        {
            x.WithName(nameof(UpdateCategoryEndpoint));
        ...
        RequestModel req, CancellationToken ct)
    {
        var query = new UpdateCategoryCommand(
            req.WorkspaceId, req.CategoryId, req.Name);
    ...
    public sealed record RequestModel(Guid WorkspaceId, Guid CategoryId, string Name);
}
